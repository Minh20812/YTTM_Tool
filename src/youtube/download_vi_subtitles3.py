...
def process_new_videos(hours=36, skip_shorts=True, min_videos_threshold=4):
    """Main function to process new videos from YouTube channels using RSS reader"""
    print(f"🚀 Starting new video processing with RSS reader (minimum {min_videos_threshold} videos required)...")
    ...
    try:
        ...
        # Step 4: Check minimum videos threshold
        if len(truly_new_videos) < min_videos_threshold:
            print(f"\n⚠️ Only {len(truly_new_videos)} new videos found, minimum required: {min_videos_threshold}")
            print("🔄 Skipping download process - threshold not met")
            print("\n" + "="*60)
            print(f"📊 PROCESSING SUMMARY:")
            print(f"   📺 Total videos from RSS: {len(new_videos)}")
            print(f"   🆕 Truly new videos: {len(truly_new_videos)}")
            print(f"   🎯 Required minimum: {min_videos_threshold}")
            print(f"   ⏰ Time window: {hours} hours")
            print(f"   🎬 Skip shorts: {skip_shorts}")
            print(f"   ⚠️ Status: THRESHOLD NOT MET - NO DOWNLOADS")
            print("="*60)
            return
        
        print(f"\n🎯 Processing {len(truly_new_videos)} truly new videos (threshold met: {min_videos_threshold})...")
        
        # Step 5: Download subtitles for new videos
        ...
        print(f"   ✅ Status: THRESHOLD MET - DOWNLOADS COMPLETED")
        print("="*60)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "--file":
            ...
            skip_shorts = True
            min_videos_threshold = 4  # Default threshold
            
            # Parse additional arguments
            i = 3
            while i < len(sys.argv):
                if sys.argv[i] == "--include-shorts":
                    skip_shorts = False
                elif sys.argv[i] == "--min-videos":
                    if i + 1 < len(sys.argv):
                        min_videos_threshold = int(sys.argv[i + 1])
                        i += 1
                i += 1
                    
            process_new_videos(hours=hours, skip_shorts=skip_shorts, min_videos_threshold=min_videos_threshold)
        else:
            print("Usage:")
            print("  python script.py                                           # Default: 36 hours, skip shorts, min 4 videos")
            print("  python script.py --hours 24                                # Custom hours, skip shorts, min 4 videos")
            print("  python script.py --hours 24 --include-shorts               # Custom hours, include shorts, min 4 videos")
            print("  python script.py --hours 24 --min-videos 6                 # Custom hours, skip shorts, min 6 videos")
            print("  python script.py --hours 24 --include-shorts --min-videos 2 # Custom hours, include shorts, min 2 videos")
            print("  python script.py --file [filename]                         # Use file fallback method")
    else:
        # Use new RSS-integrated method with default settings
        process_new_videos(hours=36, skip_shorts=True, min_videos_threshold=4)