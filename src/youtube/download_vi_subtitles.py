...
def process_new_videos():
    """Main function to process new videos from YouTube channels"""
    print("🚀 Starting new video processing...")
    
    # Đảm bảo thư mục storage tồn tại
    ensure_storage_directory()
    
    # Step 1: Get latest videos from YouTube channels
    print("📺 Fetching latest videos from YouTube channels...")
    new_videos = get_latest_links(return_links=True)
    
    if not new_videos:
        print("❌ No new videos found from YouTube channels")
        return
    
    print(f"📋 Found {len(new_videos)} videos from YouTube scan")
    ...
    print(f"   📺 Total videos from YouTube: {len(new_videos)}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--file":
        ...
        # Use new Firebase-integrated method
        process_new_videos()
...
#     print(f"🔗 {video_url}")
#     print(f"🌐 Subtitle language: {sub_lang}")
#     print(f"💾 Expected output: {video_id}.{suffix}.srt")

#     temp_output = f"{video_id}.%(ext)s"
#     raw_srt = f"{video_id}.{sub_lang}.srt"
#     final_output = f"{video_id}.{suffix}.srt"

#     if os.path.exists(final_output):
#         print(f"⚠️ Skipping (already exists): {final_output}")
#         return
...
#         else:
#             print(f"❌ Subtitle not found after download: {raw_srt}")
...
# if __name__ == "__main__":
#     download_from_list("latest_video_links.txt")
